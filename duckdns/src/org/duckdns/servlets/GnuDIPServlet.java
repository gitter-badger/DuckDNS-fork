...
public class GnuDIPServlet extends javax.servlet.http.HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		s1 = new Sender(EnvironmentUtils.getInstance().getFIXED_IP_NS1_INTERNAL(), 10025, 3, 50, 100, new BasicSender());
		s2 = new Sender(EnvironmentUtils.getInstance().getFIXED_IP_NS2_INTERNAL(), 10025, 3, 50, 100, new BasicSender());
		s3 = new Sender(EnvironmentUtils.getInstance().getFIXED_IP_NS3_INTERNAL(), 10025, 3, 50, 100, new BasicSender());
	}
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (salt != null && time != null && sign != null && user != null && domn != null && pass != null && reqc != null) {
			...
			if (domainObject != null) {
				...
				if (!domain.equals("www")) {
					...
					if (addr.equals(currentIp)) {
						...
						int result = AmazonDynamoDBDAO.getInstance().domainUpdateIp(addr, domain, domainObject.getAccountToken());
				...
				if (success) {
					if (allSame) {
						GoogleAnalyticsHelper.RecordAsyncEvent("9999",GoogleAnalyticsHelper.CATEGORY_DNS_GNUDIP,GoogleAnalyticsHelper.ACTION_UPDATE_SAME,domain,"1");
						detailedMessage = "Update - same";
						// DEBUG - we are not out of the woods yet - System.out.println("same!!");
					} else {
						GoogleAnalyticsHelper.RecordAsyncEvent("9999",GoogleAnalyticsHelper.CATEGORY_DNS_GNUDIP,GoogleAnalyticsHelper.ACTION_UPDATE,domain,"1");
			...
			if (!success) {
				GoogleAnalyticsHelper.RecordAsyncEvent("9999",GoogleAnalyticsHelper.CATEGORY_DNS_GNUDIP,GoogleAnalyticsHelper.ACTION_UPDATE_FAILED,domain,"1");

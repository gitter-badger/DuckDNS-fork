...
public class AccountServlet extends javax.servlet.http.HttpServlet {
	...
	private boolean deleteAccount(HttpServletRequest req) {
		...
		Account account = (Account) req.getSession().getAttribute("account");
		GoogleAnalyticsHelper.RecordAsyncEvent(SecurityHelper.generateEncryptedId(account.getEmail()),GoogleAnalyticsHelper.CATEGORY_ACCOUNT,GoogleAnalyticsHelper.ACTION_DELETE_ACCOUNT,"","");
	...
	private void recreateToken(HttpServletRequest req) throws IOException {
		...
		Account account = (Account) req.getSession().getAttribute("account");
		AmazonDynamoDBDAO dao = AmazonDynamoDBDAO.getInstance();
		String newToken = dao.recreateToken(account.getEmail());
		...
		GoogleAnalyticsHelper.RecordAsyncEvent(SecurityHelper.generateEncryptedId(account.getEmail()),GoogleAnalyticsHelper.CATEGORY_ACCOUNT,GoogleAnalyticsHelper.ACTION_RECREATE_TOKEN,"","");
		// TELL THEM IT WORKED

...
public class FacebookOAuth {
	...
	public static String getUserEmail(String access_token) throws ClientProtocolException, IOException {
        ...
        try {
            HttpGet httpget = new HttpGet("https://graph.facebook.com/fql?q=SELECT+uid,+name,+email+FROM+user+WHERE+uid+=+me()&access_token="+access_token);
            ...
            if (entity != null) {
                ...
                if (allObj.containsKey("data")) {
	                JSONArray array = (JSONArray) allObj.get("data");
	                if (array.size() > 0) {
	                	Map json = (Map) JSONValue.parse(array.get(0).toString());

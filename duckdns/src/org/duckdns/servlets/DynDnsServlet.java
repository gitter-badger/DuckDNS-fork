...
public class DynDnsServlet extends javax.servlet.http.HttpServlet {
	...
	private String updateTheDomains(HttpServletRequest req, String updateDomains, String token, String optionalIp) {
				...
				if (domains.size() > 0) {
					...
					for (String domain : domains) {
						...
						if (isAllowed) {
							...
							int result = AmazonDynamoDBDAO.getInstance().domainUpdateIp(optionalIp, domain, token);
		...
		if (allSame) {
			GoogleAnalyticsHelper.RecordAsyncEvent("9999",GoogleAnalyticsHelper.CATEGORY_DNS_DYN,GoogleAnalyticsHelper.ACTION_UPDATE_SAME,loggableDomains,value);
		} else {
			GoogleAnalyticsHelper.RecordAsyncEvent("9999",GoogleAnalyticsHelper.CATEGORY_DNS_DYN,GoogleAnalyticsHelper.ACTION_UPDATE,loggableDomains,value);
		}

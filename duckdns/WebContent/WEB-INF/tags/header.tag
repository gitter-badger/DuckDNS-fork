...
<c:if test="${not empty dynattrs.isLoggedIn and dynattrs.isLoggedIn eq 'true'}">
	<c:choose>
		<c:when test="${not empty dynattrs.isLoggedInPersonaStr and dynattrs.isLoggedInPersonaStr eq 'true'}">
				<div class="ee-masthead-segment__segment">
					<a id="signout" href="#" title="logout">logout</a>
				</div>
		</c:when>
		<c:otherwise>
				<div class="ee-masthead-segment__segment">
...
<c:choose>
	<c:when test="${dynattrs.isLoggedIn eq 'false'}">
				...
				<span class="ee-masthead-utility__link" >
					<input title="login with persona" type="image" id="signin" src="img/login_persona.png" class="login-button" />
				</span>	
	</c:when>
	<c:otherwise>
				<div class="ee-masthead-utility__segment ee-masthead-utility-selector ee-masthead-utility-selector--inactive">
					<span class="loggedinPrefix">logged in with&nbsp;</span><strong>${dynattrs.sessionEmail}</strong>
					<a href="#" id="accountOptionsToggle">|||</a>
					<ul id="accountOptionsHolder" class="ee-masthead-utility-selector__list">
						<li>
